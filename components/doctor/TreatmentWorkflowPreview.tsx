...
export function TreatmentWorkflowPreview({ template, planAppointments, isPatient }: TreatmentWorkflowPreviewProps) {
    return (
                        ...
                        return (
                            <div key={step.id} className="relative">
                                ...
                                <div className="flex flex-col gap-2">
                                    ...
                                    <div className="flex items-center justify-between">
                                        <Badge variant="outline" className="text-xs">{step.appointment_type}</Badge>

                                        {isPatient && status === 'pending' && (
                                            <Button size="sm" variant="outline" asChild>
                                                <Link href={`/patient/book?type=${step.appointment_type}&reason=${encodeURIComponent(step.title)}`}>
